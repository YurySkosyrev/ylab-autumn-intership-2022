...
public class BookServiceImplTest {
    ...
    void getAllBook_Test() {
        ...
        //then
        List<Book> books = bookService.getBooksByPersonId(person.getId());
    ...
    // * failed
    @Test
    @DisplayName("Ошибка при получении книги. Должно пройти успешно.")
    void failedGetBook_Test() {
        ...
        //when
        when(bookRepository.findById(bookId)).thenThrow(new NotFoundException("Book with id: 1 not found"));
        when(bookMapper.bookToBookDto(savedBook)).thenReturn(result);
    ...
    void failedGetBook1_Test() {
        ...
        assertThatThrownBy(() -> bookService.getBookById(bookId))
                .isInstanceOf(NotFoundException.class)
                .hasMessage("Book with id 1 is not found");
    }
}
